...
### EffectTileShape: стыковка хайграунда с землёй

`src/landscape_playgrounds/SDFGeneratedLandscape` раздвоён: **SDFGeneratedLandscape** — чистый стенд отладки генерации геометрии (состояние до эффектов стыковки), **EffectTileShape** — форк с эффектами стыка (общее солнце, контактное AO, shadow map, материалы стыка, grass underlay; также HiDPI-фикс сцены в точках). Геометрические кисти («Cliff 3D», «Stone 3D») и конвейеры одинаковы в обоих; эффекты — только в Effect-версии.
