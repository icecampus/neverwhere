...
Там же (`stone_field.h`) живёт `StoneField` — перенос каменного принципа на хайграунд по нодам (контракт как у `cliff::CliffField`): сетка нод → плита-плато (база `cliff::CliffField::evalBase`) → voronoi-карвинг StoneCubePlayground (`clamp(k*(F2-F1))` bulge, борозды по границам ячеек, маской прижат к поверхности плиты) + fbm → surface nets. Используется кистью «Stone 3D» в TileShapePlayground (тот же кэш/debounce, что у «Cliff 3D», свои параметры `StoneFieldParams` и серая каменная палитра через per-layer shading override; pcg3d/voronoi/fbm — бит-в-бит дубли `stone_sdf.cpp`, StoneSdf не трогаем).
