...
- tech3d-ассеты (`TechAsset`, слой `TechLandscape`, панель `TechSettings.qml`) рендерятся тем же `render_core::CliffRenderer` по шаблону stone3d: меш — через `tech::TechField` (блок `CliffParams::techField`, конвертеры `techParamsFromAssetData` в обоих доменах, ветки в `rebuildRegion`/`regionKey`/`plateauHeightOf`), а шейдинг — **без новых каналов FS**: земляная палитра TechnicalGrass едет per-asset через `CliffShading` (дефолты `Tech3dAssetData`: earth ramps, `veinThreshold=2` (veins off), `bottomDarken=0`), тёмный контур тайлов рисует существующий groove-канал (crease из поля), stone-экстры `params4` остаются нулевыми, rim=0. Сборка кадра — `WorldFrame::techTiles` + `WorldRenderer::ensureTechAsset`; tech-тайлы входят в тот же объединённый вызов cliff-пасса (cliff+stone+tech одним `render`) и в stitch-футпринт AO. Параметр `outlineDepth > 0` включает береговую линию (outline-кольцо с подводной частью, см. tech_field; bbox в `rebuildRegion` тогда с маржой 2). Бандлы-примеры: `resources/assets/landscape/TechGrass` (суше) и `TechGrassOutline` (берег, `outlineDepth=1`).
