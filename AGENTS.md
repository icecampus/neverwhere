...
**Z-буфер SDFGeneratedLandscape (грабли).** Глубина в плоском iso печётся из **ground-y** точки (`bakedDepth(fieldY)` в `AtlasRenderer.cpp`): field-y растёт к зрителю, поэтому при LESS_EQUAL + clear 1.0 ближнее — меньшее z. Анкер обязан быть **константой, а не от камеры**: cliff/stone/tech-стримы пекут z один раз при (дебаунсенном) ребилде меша, а оверлей пересчитывается каждый кадр — камеро-зависимый анкер расходится между ними, и панорамирования на одну клетку хватало, чтобы сетка перескочила из-под меша наверх (у HighgroundWithEffects та же константа `kZFar`/`kZScale`). Второй момент: у 3D-геометрии есть части **ниже плоскости земли** (стенки цокольной плиты, подводная нога береговой линии Tech 3D Outline) — по ground-y они дальше плоскости, и сетка ромбов ближних клеток резала их насквозь. Поэтому оверлей (сетка, hover-футпринт) сидит на константе `kOverlayDepth` **позади всего диапазона глубин меша**: любой фрагмент 3D перекрывает оверлей, а на пустой земле сетка видна как раньше (плоские тайлы глубину не пишут).
