...
## Play-Test в редакторе
Редактор умеет **запускать EpicGameClient-эквивалент прямо внутри себя**: кнопка ▶
поверх канваса карты открывает игровую вкладку с изолированным `game_runtime::Runtime`
и сессией из фикстуры (мок-профиль — `newGame()`, управление моками — позже).
Одна игровая вкладка на главу; повторный Play перезапускает сессию.
