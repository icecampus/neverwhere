...
world-space UV (репит каждые 256 px, LINEAR+REPEAT). Цепочки строятся обходом
...
соседнюю петлю «задом наперёд»); figure-eight полилинии режутся на простые
петли по повторным вершинам; дырки (обратная ориентация) вливаются во
внешнюю петлю zero-width мостом (`mergeHoleIntoOuter`) — яма остаётся открытой.
Fallback — треугольники по маске нод per-cell (не-rock стены, сбой
триангуляции), без `topTexture` — атласные квады.
