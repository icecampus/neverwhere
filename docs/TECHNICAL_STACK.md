...
- **Сброс GL-состояния для Qt:** после Sokol-рендера возвращать критичные состояния
  ...
  Обязательно и `glDisable(GL_FRAMEBUFFER_SRGB)`: GLCORE-бэкенд sokol жёстко включает
  его на старте каждого offscreen-пасса («crude hack» в `_sg_gl_begin_pass`) и не
  восстанавливает — на macOS оконный drawable sRGB-кодирован, поэтому весь
  последующий рендер Qt (весь QML-UI + композитинг нашего FBO) получает вторую
  linear→sRGB конверсию и всё окно засвечивается (панель #1A1A1A читается как
  (90,90,90)). На Windows/Linux default framebuffer LINEAR-кодирован, флаг там
  безвреден — баг виден только на macOS.
...
PolygonalGeneratedLandscape (слой «Cyclopean 3D» из
...
Спрайты (Tile2D) тестируют общий z-буфер (вертикальная плоскость на базовой
линии, `LESS_EQUAL` без записи, см. `render_core/depth_levels.h`), поэтому с
поднятой геометрией они пересекаются корректно per-pixel. Оставшееся
ограничение: якорь спрайта всегда на уровне 0 — дерево, стоящее НА поднятой
клетке, не поднимается на плато (нужен поднятый якорь по высоте клетки) —
отдельная задача.
