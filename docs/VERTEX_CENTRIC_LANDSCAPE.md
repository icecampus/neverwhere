...
- **Рисование узлов в редакторе как первичной операции.** Кисть уже оперирует
  узлами (`buildLandNodes`/`tileTypeAt`), но UX «нарисуй рельеф пером»
  ограничен; часть пайплайна всё ещё завязана на выбор тайлов.
