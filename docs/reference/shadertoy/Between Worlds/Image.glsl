...
vec3 tex3D( sampler2D tex, in vec3 p, in vec3 n ){
	...
	return 1.-(texture(tex, p.yz)*n.x + texture(tex, p.zx)*n.y + texture(tex, p.xy)*n.z).xyz;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
	...
	// Camera Setup.
	vec3 lookAt = vec3(iTime*4.+1., 0.0, iTime*2.+ 1.);  // "Look At" position.
	...
	rd.xy *= rot2( smoothstep(0.3, 0.7, sin(iTime*0.1875 - 2.)*0.5+0.5)*3.14159 + sin(iTime*0.1875 -2.)*0.1 );
