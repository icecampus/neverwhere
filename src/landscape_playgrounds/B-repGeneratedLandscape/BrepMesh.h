// Playground fork of src/libs/landscape_mesh (bit-for-bit copy at fork time,
// namespace landscape_mesh -> brepmesh). B-repGeneratedLandscape iterates on
// the highground generation HERE — the shared library stays untouched, so the
// editor's CyclopeanRenderer and other playgrounds are unaffected. Diverge
// freely; do not merge back blindly.
#pragma once
