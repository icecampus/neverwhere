...
// same GLSL/HLSL/MSL triple SDFGeneratedLandscape carries. The vertex shader
...
// gotcha: overlay vertices are re-projected every frame, while future 3D mesh
// streams bake z once per rebuild — a camera-dependent anchor would let the
// grid hop over/under the meshes on pan).
...
void GridRenderer::init() {
    ...
    // The overlay shares the z-buffer convention of the 3D passes (baked
    ...
    pip.label = "fencepath-color-line-pip";
