...
TechOutlineField::TechOutlineField(
    const TechOutlineFieldParams& params,
...
float TechOutlineField::nodeAt(int x, int z) const {
    return (x >= 0 && x < m_nodesX && z >= 0 && z < m_nodesY)
        ? m_values[static_cast<size_t>(z) * m_nodesX + x]
        : 0.0f;
}

bool TechOutlineField::cellNonEmpty(int x, int z) const {
...
float TechOutlineField::centerHeight(float maxV, float minV, int nMax, bool opposite) const {
...
float TechOutlineField::heightAt(float x, float z) const {
    return heightAt(x, z, nullptr);
}

float TechOutlineField::heightAt(float x, float z, float* outGradLen) const {
...
float TechOutlineField::baseDepth() const {
...
float TechOutlineField::eval(const glm::vec3& p) const {
    float gradLen = 0.0f;
...
float TechOutlineField::grooveDepth(const glm::vec3& p) const {
...
void TechOutlineField::sample(std::vector<float>& outValues) const {
...
cliff::ScalarFieldView TechOutlineField::view() const {
