...
void AtlasRenderer::render(
    ...
    // Cliff/stone/tech layers: scalar-field surface nets from the same nodes,
    // drawn with the z-buffer and per-pixel shading. The mesh is cached per
    // brush; the heavy field rebuild runs at most once per edit burst (0.3 s
...
void AtlasRenderer::rebuildCliffCache(
    ...
    if (cache.meshDirty) {
        ...
        if (onNodes.empty()) {
            ...
            const int maskRing = (cache.mask && cache.maskParams.spreadDistance > 0.0f)
                ? 1 + static_cast<int>(std::ceil(cache.maskParams.spreadDistance))
                : 1;
            const int margin = shoreline ? 2 : maskRing;
            minX -= margin;
            minY -= margin;
            maxX += margin;
            maxY += margin;
            ...
            if (cache.stone) {
                ...
                // TechnicalGrass ridge/valley heightfield as real geometry:
                // TechField -> generic ScalarFieldView -> surface nets.
                tech::TechField field(cache.techParams, nodes.data(), nodesX, nodesY);
                cliff::ScalarFieldView view = field.view();
                std::vector<float> samples;
                field.sample(samples);
                cliff::regularizeSigns(view, samples, &regStats);
                ...
                // The minimal teaching sample — one axis-aligned box per
                // painted cell: BoxField -> generic ScalarFieldView -> nets.
                boxfield::BoxField field(cache.boxParams, nodes.data(), nodesX, nodesY);
                cliff::ScalarFieldView view = field.view();
                std::vector<float> samples;
                field.sample(samples);
                cliff::regularizeSigns(view, samples, &regStats);
                cache.mesh = cliff::extractSurfaceNets(view, samples, nullptr);
                cache.stats.voxelCount = view.nx * view.ny * view.nz;
            } else if (cache.circle) {
                // The second teaching sample — a cylinder per painted node
                // plus a full parallelepiped per fully-painted cell:
                // CircleField -> generic ScalarFieldView -> surface nets.
                circlefield::CircleField field(cache.circleParams, nodes.data(), nodesX, nodesY);
                cliff::ScalarFieldView view = field.view();
                std::vector<float> samples;
                field.sample(samples);
                cliff::regularizeSigns(view, samples, &regStats);
