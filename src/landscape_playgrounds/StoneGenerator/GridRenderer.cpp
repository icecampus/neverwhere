...
// gotcha: overlay vertices are re-projected every frame, while a 3D mesh
...
// Small diamond centered on a vertex-node (seed marker / hover node).
void appendNodeDiamond(
    std::vector<GridColorVertex>& out,
    const topology_core::DiamondIsometry& iso,
    glm::ivec2 node,
    glm::vec4 color,
    ...
    const glm::vec2 c = iso.nodeToField(node);
    const glm::vec2 half = iso.dims.cellSize() * 0.5f * scale;
...
void GridRenderer::init() {
    ...
    // The overlay shares the z-buffer convention of a future 3D pass (baked
...
void GridRenderer::render(
    ...
    // One stream, two draws: fills first (triangle pipeline), then all the
    // line vertices (line pipeline). A single buffer upload feeds both
    ...
    const bool hoverInMap = hasHover && hoverNode.x >= 0 && hoverNode.y >= 0 &&
        hoverNode.x <= mapW && hoverNode.y <= mapH;

    if (hoverInMap) {
        // The four cells that share the hovered vertex-node: a faint
        // footprint so the brush target is readable without a mesh.
        for (const glm::ivec2 cell : topology_core::DiamondIsometry::nodeNeighbourCells(hoverNode)) {
            if (cell.x >= 0 && cell.y >= 0 && cell.x < mapW && cell.y < mapH) {
                appendDiamondFill(verts, iso, cell, {1.0f, 0.75f, 0.25f, 0.12f});
            }
        }
    }

    if (nodes && nodeW > 0 && nodeH > 0) {
        for (int y = 0; y < nodeH; ++y) {
            for (int x = 0; x < nodeW; ++x) {
                if (nodes[static_cast<std::size_t>(y) * nodeW + x] == 0) {
                    continue;
                }
                appendNodeDiamond(verts, iso, {x, y}, {0.62f, 0.56f, 0.48f, 0.90f}, 0.28f, false);
            }
        }
    }

    if (hoverInMap) {
        appendNodeDiamond(verts, iso, hoverNode, {1.0f, 0.75f, 0.25f, 0.95f}, 0.34f, false);
    }

    const int fillVertCount = static_cast<int>(verts.size());
    ...
    if (hoverInMap) {
        appendNodeDiamond(verts, iso, hoverNode, {1.0f, 0.75f, 0.25f, 0.95f}, 0.34f, true);
    }
