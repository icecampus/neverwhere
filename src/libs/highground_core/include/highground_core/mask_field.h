// Mask field: a node-driven scalar field that takes the 2D silhouette a
// painted node mask draws — per-node fill (1 at an on-node, 0 at an
// off-node) interpolated across each cell, mask = fill >= 0.5 — and
// extrudes that footprint into a thin plate with a sloped skirt around it:
//
...
//   water line): the slab spans y = -botH..+topH and the skirt foot reaches
//   y = -botH exactly at s = S.
//
//   With a reliefMap bound, the iso surface is then shifted by the centered
//   tiling height sample:
//
//   eval(p) -= reliefDepth * (relief(x,z) - 0.5) * 2 * fade(-s / reliefFade)
//
//   — the displacement-map dunes become real geometry with a silhouette
//   (surface nets extracts them), not a fragment trick. The raster is
//   low-passed at load (see reliefMapFromImage): the voxel grid cannot hold
//   the fine ripples of the source map (they live in the material's normal
//   map), only the dune-scale undulation. The fade keeps the walls and the
//   skirt clean. At reliefDepth = 0 or without a map the field is
...
//   spreadDistance > 0 grows no full-height geometry — instead the plate's
//   edge rolls off as a height ramp driven by the distance from the core
//   contour, with the iso lines (and so the skirt foot) rounded around
...
//   to a slab with a vertical wall on the fill = 0.5 contour (edges cross
//   at their midpoints, half-painted cells become wedges, a lone node
//   becomes a small blob).
//
// Unlike cliff::CliffField there is no neighbourhood loop at all: the
...
//
// Graduated from the SDFWithMaterialLandscape playground (the "Mask 3D"
// brush) into the shared library for the editor's mask3d assets.
//
// Same contract as cliff::CliffField / tech::TechField: node grid + params
...
// Builds a relief raster from a grayscale image (row-major, one byte per
// texel at the given stride — 1 for gray8, 4 for RGBA8 taking R). The
// raster is low-passed by repeated 2x2 box halving with REPEAT wrap (stays
// tileable) down to ~32x16: geometry relief carries only the dune-scale
// undulation, finer ripples would alias at voxel resolution (and already
// live in the material's normal map). Returns an empty map on bad input.
...
struct MaskFieldParams {
    float cellSize = 0.04f;      // field voxel size in world units (finer than
                                 // the cliff default 0.06: the micro relief
                                 // needs a few voxels per dune to show)
    ...
    std::uint32_t reliefVersion = 0;      // callers that cache by memcmp bump
                                          // this on map (re)load so the change
                                          // is seen (the playground debounce)
...
public:
    ...
    // surface above y = -height*sink within s <= spreadDistance, positive
