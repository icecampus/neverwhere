...
StoneAsset::StoneAsset(QObject* parent):
    SliceAsset(AssetTypes::stone3d, parent)
{

}

void StoneAsset::ensureStoneData()
{
    if (!data.stone3dData)
    {
...
void StoneAsset::load(const BaseData::AssetData& data_)
{
    Asset::load(data_);
    ...
    if (!data.stone3dData->thumbnail.empty())
    {
        loadAtlasFiles(data.root() / data.stone3dData->thumbnail, {});
    }
}

QVariantMap StoneAsset::stoneParams() const
{
    QVariantMap out;
    if (!data.stone3dData)
    {
    ...
    const BaseData::Stone3dAssetData& d = *data.stone3dData;
    ...
    out["voroScale"] = d.voroScale;
    out["cellJitter"] = d.cellJitter;
    out["grooveDepth"] = d.grooveDepth;
    out["grooveK"] = d.grooveK;
    out["seed"] = d.seed;
    out["flatTop"] = d.flatTop;
    out["flatTopLo"] = d.flatTopLo;
    out["flatTopHi"] = d.flatTopHi;
    out["rimWidth"] = d.rimWidth;
    out["rimBulge"] = d.rimBulge;
    out["rimNotch"] = d.rimNotch;
    ...
    out["grassFade"] = d.grassFade;
    out["rimShade"] = d.rimShade;
    out["topTexMix"] = d.topTexMix;
    out["topTexTiles"] = d.topTexTiles;
    return out;
}

void StoneAsset::setStoneParam(const QString& name, const QVariant& value)
{
    ensureStoneData();
    BaseData::Stone3dAssetData& d = *data.stone3dData;
    ...
    else if (name == "voroScale") d.voroScale = value.toFloat();
    else if (name == "cellJitter") d.cellJitter = value.toFloat();
    else if (name == "grooveDepth") d.grooveDepth = value.toFloat();
    else if (name == "grooveK") d.grooveK = value.toFloat();
    else if (name == "seed") d.seed = value.toFloat();
    else if (name == "flatTop") d.flatTop = value.toBool();
    else if (name == "flatTopLo") d.flatTopLo = value.toFloat();
    else if (name == "flatTopHi") d.flatTopHi = value.toFloat();
    else if (name == "rimWidth") d.rimWidth = value.toFloat();
    else if (name == "rimBulge") d.rimBulge = value.toFloat();
    else if (name == "rimNotch") d.rimNotch = value.toFloat();
    ...
    else if (name == "grassFade") d.grassFade = value.toFloat();
    else if (name == "rimShade") d.rimShade = value.toFloat();
    else if (name == "topTexMix") d.topTexMix = value.toFloat();
    else if (name == "topTexTiles") d.topTexTiles = value.toFloat();
}
