...
// ---------------------------------------------------------------------------
// GLSL (порт MSL-версии: thread& -> inout, constant FsParams& -> uniform,
// floatN -> vecN, float3x3 -> mat3 с тем же column-major порядком скаляров)
// ---------------------------------------------------------------------------
...
// Глобалы dr2-шейдера (ltDir/elAx/tCur/dstFar/idObj) живут в контексте,
// как и в MSL-версии.
struct SceneCtx {
    vec3 ltDir;
    vec3 elAx;
...
// GLSL-семантика mod (floor-based) совпадает с встроенной, но оставляем
// gmod для бит-в-бит паритета с MSL/HLSL портами.
float gmod(float x, float y) { return x - y * floor(x / y); }
vec2 gmod(vec2 x, float y) { return x - y * floor(x / y); }
...
float ObjDf(inout SceneCtx ctx, vec3 p)
{
  vec3 q;
  float dMin, d;
  dMin = ctx.dstFar;
  q = p;
  d = PrEllipsDf(q, ctx.elAx + vec3(0.05, 0., 0.05));
  q = p;
  q.xy = Rot2D(q.xy, pi / 5.);
  d = SmoothMax(d, abs(gmod(q.y + 0.1, 0.4) - 0.2) - 0.1, 0.02);
  q = p;
  ...
  q = p;
  q -= vec3(2.1, -0.05, 0.);
  d = PrBoxDf(q, vec3(3.4, 0.05, 1.3));
  DMIN(ctx, 2);
  q = p;
  q -= vec3(4.2, 1., 0.);
  d = PrBoxDf(q, vec3(1., 1., 1.));
  // Спиральные борозды как у омфалоса, но в локальных координатах куба;
  // SmoothMin с внутренним кубом — аналог внутреннего эллипсоида: борозды
  // режут только оболочку, ядро остаётся целым.
  vec3 qc;
  ...
  qc.xy = Rot2D(qc.xy, 1.1 * pi / 5.);
  d = SmoothMax(d, abs(gmod(qc.y + 0.1, 0.4) - 0.2) - 0.1, 0.02);
  d = SmoothMin(d, PrBoxDf(q, vec3(0.82, 0.82, 0.82)), 0.1);
  DMIN(ctx, 3);
  return dMin;
}

float ObjRay(inout SceneCtx ctx, vec3 ro, vec3 rd)
{
  float dHit, d;
  dHit = 0.;
  for (int j = 0; j < 120; j++) {
...
vec3 ObjNf(inout SceneCtx ctx, vec3 p)
{
  vec4 v;
  vec2 e = vec2(0.0002, -0.0002);
  v = vec4(-ObjDf(ctx, p + e.xxx), ObjDf(ctx, p + e.xyy), ObjDf(ctx, p + e.yxy), ObjDf(ctx, p + e.yyx));
  return normalize(2. * v.yzw - dot(v, vec4(1.)));
}

float ObjSShadow(inout SceneCtx ctx, vec3 ro, vec3 rd)
{
  float sh, d, h;
  sh = 1.;
  d = 0.02;
...
vec3 ShowScene(inout SceneCtx ctx, vec3 ro, vec3 rd)
{
  vec4 col4;
  vec3 col, vn;
  float dstObj, sh, f;
  ...
  if (dstObj < ctx.dstFar) {
    ...
    if (ctx.idObj == 1) {
      ...
      col4 = mix(vec4(0.4, 0.4, 0.45, 0.05), vec4(0.6, 0.5, 0.5, 0.3) +
         vec4(1., 1., 0.5, 0.5) * step(0.8, f),
         smoothstep(0.87, 0.95, length(ro - vec3(4.2, 1., 0.)))) * (1. - 0.3 * f);
    } else if (ctx.idObj == 2) {
...
vec4 mainImage(inout SceneCtx ctx, vec2 fragCoord)
{
  mat3 vuMat;
  vec4 mPtr;
  vec3 ro, rd, col;
  ...
  zmFac = 6.;
  ctx.dstFar = 40.;
  ctx.ltDir = vuMat * normalize(vec3(1., 1., -1.));
#if ! AA
  const float naa = 1.;
#else
  const float naa = 3.;
#endif
  col = vec3(0.);
  sr = 2. * gmod(dot(gmod(floor(0.5 * (uv + 1.) * canvas), 2.), vec2(1.)), 2.) - 1.;
  for (float a = 0.; a < naa; a++) {
    rd = vuMat * normalize(vec3(uv + step(1.5, naa) * Rot2D(vec2(0.5 / canvas.y, 0.),
       sr * (0.667 * a + 0.5) * pi), zmFac));
    col += (1. / naa) * ShowScene(ctx, ro, rd);
  }
  return vec4(pow(col, vec3(0.8)), 1.);
}
...
vec4 Hashv4v3(vec3 p)
{
  ...
  return fract(sin(vec4(dot(p, cHashVA3), dot(p + vec3(1., 0., 0.), cHashVA3),
     dot(p + vec3(0., 1., 0.), cHashVA3), dot(p + vec3(0., 0., 1.), cHashVA3))) * cHashM);
}
...
{
  vec3 g;
  g = vec3(Fbmn(p + vec3(0.1, 0., 0.), n), Fbmn(p + vec3(0., 0.1, 0.), n),
     Fbmn(p + vec3(0., 0., 0.1), n)) - Fbmn(p, n);
  return normalize(n + f * (g - n * dot(n, g)));
}

void main() {
    SceneCtx ctx;
    frag_color = mainImage(ctx, v_uv * resolution);
}
)";
