...
Vec3 triNormalPreview(const Vec3& a, const Vec3& b, const Vec3& c) {
...
void drawRockFractureDebugView(const RockFractureSettings& settings, const RockFractureModel& model, const ImVec2& viewportSize) {
    ...
    drawList->AddRectFilled(origin, {origin.x + viewportSize.x, origin.y + viewportSize.y}, IM_COL32(20, 23, 28, 255));
    drawList->AddRect(origin, {origin.x + viewportSize.x, origin.y + viewportSize.y}, IM_COL32(70, 78, 92, 255));
    drawList->AddText({origin.x + 12.0f, origin.y + 12.0f}, IM_COL32(220, 228, 240, 255), "Rock Fracture: top-down XZ debug view");
    drawList->PushClipRect(origin, {origin.x + viewportSize.x, origin.y + viewportSize.y}, true);
    ...
    // Determine the world bounds. We use the tile as defined by the settings
    // (half-extent = tileSize/2) and recenter the layout to the viewport.
...
void drawRockFractureMeshPreview(const RockFractureSettings& settings, const RockFractureModel& model, const ImVec2& viewportSize) {
    const ImVec2 origin = ImGui::GetCursorScreenPos();
    ImGui::InvisibleButton("##RockFractureViewport", viewportSize);
    const bool hovered = ImGui::IsItemHovered();
    const bool dragging = ImGui::IsItemActive() && ImGui::IsMouseDragging(ImGuiMouseButton_Left);
    ImGuiIO& io = ImGui::GetIO();

    {
        std::lock_guard<std::mutex> lock(g_stateMutex);
        if (hovered && io.MouseWheel != 0.0f) {
            g_rockCamera.zoom = clampFloat(g_rockCamera.zoom * (1.0f + io.MouseWheel * 0.12f), 0.25f, 16.0f);
        }
        if (dragging) {
            g_rockCamera.pan.x += io.MouseDelta.x;
            g_rockCamera.pan.y += io.MouseDelta.y;
        }
        if (hovered && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left)) {
            g_rockCamera.zoom = 1.0f;
            g_rockCamera.pan = {0.0f, 0.0f};
        }
    }
    ...
    // Same isometric projection as drawObjectMesh3dPreview.
    ...
    // Sort triangles by average Y so back faces are drawn first (painter's algorithm).
    ...
    const Vec3 lightDir = normalize({-0.45f, 0.85f, -0.35f});
    auto shade = [&](ImU32 color, const Vec3& n) {
        const float lambert = std::max(0.0f, dot(n, lightDir));
        return shadeColor(color, 0.55f + lambert * 0.55f);
    };
    ...
    for (int index : drawOrder) {
        ...
        const Vec3 n = triNormalPreview(v0, v1, v2);
        const ImVec2 s0 = project(v0);
        const ImVec2 s1 = project(v1);
        const ImVec2 s2 = project(v2);
        const ImU32 color = shade(IM_COL32(168, 156, 138, 255), n);
    ...
    char titleText[160];
    snprintf(titleText, sizeof(titleText), "Rock Fracture mesh: %s (tri=%d, v=%d)",
        rockFractureKindName(settings.kind), model.triangleCount, model.vertexCount);
    drawList->AddText({origin.x + 12.0f, origin.y + 12.0f}, IM_COL32(220, 228, 240, 255), titleText);
