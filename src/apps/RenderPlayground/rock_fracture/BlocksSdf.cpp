/*
 Alternative implementation ported from Code/Source/blocks-sdf.cpp.
 Identical to blocks.cpp except SDFUnionSphereLOD does NOT extend the union box
 by the transition radius. Preserved verbatim from the original for reference,
 disabled in the default build to avoid ODR violations with Blocks.cpp.

 Enable by removing the #if 0 guard and compiling this file in place of
 Blocks.cpp (see CMakeLists.txt).
*/
