...
inline double QuinticSmooth(double t) {
    return std::pow(t, 3.0) * (t * (t * 6.0 - 15.0) + 10.0);
}
...
public:
    ...
    inline double Max() const { return Math::Max(Math::Max(x, y), z); }
    inline double Min() const { return Math::Min(Math::Min(x, y), z); }
...
public:
    ...
    Vector2 operator-= (const Vector2& v) { return Vector2(x - v.x, y - v.y); }
    Vector2 operator*= (double f) { return Vector2(x * f, y * f); }
    Vector2 operator/= (double f) { return Vector2(x / f, y / f); }
    ...
    inline Vector3 ToVector3(double yy) const { return Vector3(x, yy, y); }
    inline double Max() const { return Math::Max(x, y); }
    inline double Min() const { return Math::Min(x, y); }
};
...
inline double SquaredMagnitude(const Vector2& u) { return u.x * u.x + u.y * u.y; }
inline Vector2 Normalize(const Vector2& v) { double kk = 1.0 / Magnitude(v); return v * kk; }
...
inline double& Matrix4::operator() (int i, int j) { return r[i + j + j + j]; }
inline double Matrix4::operator() (int i, int j) const { return r[i + j + j + j]; }
inline double Matrix4::Determinant() const {
    const Matrix4& M = *this;
    return M(0, 0) * Matrix3(M(1, 1), M(1, 2), M(1, 3),
                             M(2, 1), M(2, 2), M(2, 3),
                             M(3, 1), M(3, 2), M(3, 3)).Determinant()
         - M(1, 0) * Matrix3(M(0, 1), M(0, 2), M(0, 3),
                             M(2, 1), M(2, 2), M(2, 3),
                             M(3, 1), M(3, 2), M(3, 3)).Determinant()
         + M(2, 0) * Matrix3(M(0, 1), M(0, 2), M(0, 3),
                             M(1, 1), M(1, 2), M(1, 3),
                             M(3, 1), M(3, 2), M(3, 3)).Determinant()
         - M(3, 0) * Matrix3(M(0, 1), M(0, 2), M(0, 3),
                             M(1, 1), M(1, 2), M(1, 3),
