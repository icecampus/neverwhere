...
void marching_cube_polygonize(const TTree* tree, int res, MarchingCubeMesh& out)
{
	vertices.clear();
	indices.clear();
	voxels.clear();
	...
	out.normals.reserve(vertices.size() * 3);
	...
	for (const Vertex& v : vertices) {
		...
		out.normals.push_back(v.normal.x);
		out.normals.push_back(v.normal.y);
		out.normals.push_back(v.normal.z);
	}
	for (int idx : indices) {
		out.indices.push_back((std::uint32_t)idx);
	}
}
...
void marching_cube(const char* url, const TTree* tree, int res)
{
	...
	marching_cube_polygonize(tree, res, mesh);
	...
	for (std::size_t i = 0; i + 2 < mesh.indices.size(); i += 3) {
		out << "f " << mesh.indices[i] + 1 << "//" << mesh.indices[i] + 1
