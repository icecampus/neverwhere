...
def generate_png_atlas_material(output_path: str, style: MaterialStyle, repo_root: str) -> None:
    ...
    masks = {
        ...
        "UpCorner": [0, 0.5, 1, 0.5, 0, 0, 0, 0, 0.5],
        "RightCorner": [0, 0, 0, 0.5, 1, 0.5, 0, 0, 0.5],
        "DownCorner": [0, 0, 0, 0, 0, 0.5, 1, 0.5, 0.5],
        "LeftCorner": [1, 0.5, 0, 0, 0, 0, 0, 0.5, 0.5],
    ...
    for i in range(COLS * ROWS):
        ...
        if style.surface_uv_jitter_px > 0:
            ...
            tile_surface = tile_pattern(
                surface_tex, (TILE_W, TILE_H), offset=(rng.randrange(-j, j + 1), rng.randrange(-j, j + 1))
            )
        else:
            tile_surface = tile_pattern(surface_tex, (TILE_W, TILE_H), offset=(i * 7, i * 11))
    ...
    print(f"Material atlas (Ridge) saved to {output_path}")


def main() -> int:
    ap = argparse.ArgumentParser(description="Generate material atlas (Ridge variant). Writes temp output and publishes to resources.")
    ...
    if args.output:
        ...
        out_dir = os.path.join(args.temp_root, "material", "ridge", run_id)
        ensure_dir(out_dir)
    ...
    if not args.no_publish:
        publish_atlas_and_thumbnail(
            ...
            pack_dir=os.path.join("resources", "assets", "landscape", "MaterialGrassRidge"),
            cols=4,
            rows=6,
            thumb_width=args.thumb_width,
            ensure_index=True,
        )
        print("Published to resources/assets/landscape/MaterialGrassRidge (atlas.png + thumbnail.png + index.json)")
    return 0
