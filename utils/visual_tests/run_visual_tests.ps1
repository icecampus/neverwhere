...
$ExePath = Join-Path $RepoRoot "_intermediate_64\Debug\PolygonalGeneratedLandscape.exe"
$PythonScript = Join-Path $RepoRoot "utils\visual_tests\check_mesh_preview.py"

if (-not (Test-Path $ExePath)) {
    throw "Build PolygonalGeneratedLandscape first: $ExePath"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "PolygonalGeneratedLandscape --capture-visuals failed with exit code $LASTEXITCODE"
}
