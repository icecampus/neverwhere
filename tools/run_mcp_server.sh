#!/bin/sh
# macOS launcher for the neverwhere debug MCP server.
#
...
VENV="$REPO_ROOT/tools/debug_mcp/.venv"
...
if [ ! -x "$VENV_PY" ]; then
    BASE_PY="$(_pick_python)" || {
        echo "run_mcp_server.sh: no suitable python3 found (need Homebrew python3, not /usr/bin/python3)" >&2
        exit 1
    }
    echo "run_mcp_server.sh: creating venv $VENV with $BASE_PY" >&2
    "$BASE_PY" -m venv "$VENV"
    "$VENV/bin/pip" install -q -r "$REPO_ROOT/tools/debug_mcp/requirements.txt"
elif ! "$VENV_PY" -c "import mcp" >/dev/null 2>&1; then
    # Venv exists but deps are missing/broken — repair.
    "$VENV/bin/pip" install -q -r "$REPO_ROOT/tools/debug_mcp/requirements.txt"
fi

exec "$VENV_PY" -m tools.debug_mcp.server
